...
public class Lotes implements Serializable {
    ...
    public void imprimirTodosLotes(Lotes lotes) {
        ArrayList<Ativo>[] todosLotes = lotes.getLotes();

        for (int i = 0; i < todosLotes.length; i++) {
            System.out.println("Lote " + TipoLote.values()[i] + ":");
            ArrayList<Ativo> ativos = todosLotes[i];

            if (ativos.isEmpty()) {
                System.out.println("  (vazio)");
            } else {
                for (Ativo ativo : ativos) {
                    System.out.println("  - " + ativo.getTipo());
                }
            }
        }
    }
